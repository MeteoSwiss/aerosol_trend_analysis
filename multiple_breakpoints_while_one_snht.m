function result = multiple_breakpoints_while_one_snht(data,param, nb_data_min, alpha)
% multiple_breakpoints iteratively search for breakpoints in a time series
% (data) with a defined method (e.g. SNHT). The minimal used period and/or the number of iterations can
% be given.
%   Input: data
%           method
%         nb_data_min
...
result=struct("level",NaN(nb_preallocation,1),"pvalue",NaN(nb_preallocation,1),"time",NaT(nb_preallocation,1),"PrctDiff",NaN(nb_preallocation,3));
        ...
        if ~isempty(tree{k})
            if length(tree{k}.(param{1}))>=nb_data_min
                ...
                if ~isnat(ttt)
                    ...
                    tree{2*k}=tree{k}(tree{k}.Time<ttt,:);
                    tree{2*k+1}=tree{k}(tree{k}.Time>ttt,:);
