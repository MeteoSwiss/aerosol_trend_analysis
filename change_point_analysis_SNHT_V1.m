...
for i=1:length(param)
    ...
    % % result_m = multiple_breakpoints_while_one_snht(data_m,{param{i}}, 12, 0.05);
    ...
    result_m = multiple_breakpoints_while_one_snht(data_m_residue,{param{i}}, 12, 0.05);
    ...
    result_m = multiple_breakpoints_while_one_snht(data_m_residueLog,{param{i}}, 12, 0.05);
...
% % backward direction (inv), multiple break points
 ...
 result_m = multiple_breakpoints_while_one_snht(data_m_residueLog,{strcat(param{i},'_inv')}, 12, 0.05);
...
% % Pettitt from all 2 y periods in forward direction only
 ...
 % % %    Tresult(4*length(param)+i,:)=table({station},start_time, end_time,period, {granu}, {strcat(param{i},'_residueLog','_2yper')} ,{inst},  {'Pettitt'}, {result_m},'VariableNames',{'station','start_time','end_time','length_period','granularity','parameter','instrument','method','results'});
    ...
    p2=plot(selectx.time+days(30/2),selectx.pvalue,'o','MarkerSize',grandeur(i), 'MarkerEdgeColor',couleur( i), 'LineWidth',1.5); %'MarkerFaceColor',couleur(i),
    ...
    p3=plot(selectx.time+days(30/2),selectx.pvalue,'s','MarkerSize',grandeur(i), 'MarkerEdgeColor',couleur( i), 'LineWidth',1.5);
    ...
    p4=plot(selectx.time+days(30/2),selectx.pvalue,'^','MarkerSize',grandeur(i), 'MarkerEdgeColor',couleur( i), 'LineWidth',1.5);
    ...
    p5=plot(selectx.time+days(30/2),selectx.pvalue,'v','MarkerSize',grandeur(i), 'MarkerEdgeColor',couleur( i), 'LineWidth',1.5);
...
if length(param)>1
    for i=1:length(param)-1
       ...
       subplot(nb_subplot,1,4);
            ...
            ylabel(inst);
            yyaxis right
            p9=plot(result_m.time+days(30/2),result_m.pvalue,'vk','MarkerSize',12,'LineWidth',1,'Color',couleur(i));
            ylim([0, alpha]);
            ylabel('p-value');
