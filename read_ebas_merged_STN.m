...
STN_rd_h=Timetable_naming_trend_inst(merged_data);
...
% compute dry scat and backscat with hourly data with RH >50% being removed
names_var=fieldnames(STN_rd_h);

RH=names_var(contains(names_var,'U'));
%select RH too high

indRH=STN_rd_h.(RH{1})>50 ;

% select variables associated with RH measurements

S1=names_var(startsWith(names_var,["Bs","Bbs"]));
for j=1:length(S1)
    S=strcat((S1{j}),'_dry');
    STN_rd_h.(S)=STN_rd_h.(S1{j});
    STN_rd_h.(S)(indRH)=NaN;
end
